...
/** Module 8 — Mes entretiens techniques : configuration des disponibilites,
 *  proposition dirigee de creneaux a un candidat precis (Etape 8 du CDC),
 *  et verdict GO/NO-GO qui fait avancer le pipeline vers FINAL_REVIEW. */
@Component({
  selector: 'app-technique-entretiens',
  standalone: true,
    ...
    <div class="gcol">
      @for (a of interviewApplications(); track a.id) {
        <div class="card">
          ...
          <div style="background:var(--gray-50);border-radius:var(--r-md);padding:14px;margin-top:10px">
            <div class="fg">
              <label class="fl">Compte-rendu de l'entretien</label>
              <textarea class="fi fi-ta" [(ngModel)]="reports[a.id]" [ngModelOptions]="{standalone:true}"
                        placeholder="Compétences validées, points d'attention..."></textarea>
            </div>
            <div style="display:flex;gap:8px;justify-content:flex-end">
              <button class="btn btn-grn" (click)="submitOutcome(a, 'GO')" [disabled]="processingId() === a.id">
                ✓ GO — Compétences validées
              </button>
              <button class="btn btn-dng" (click)="submitOutcome(a, 'NO_GO')" [disabled]="processingId() === a.id">
                ✗ NO-GO — Expertise insuffisante
              </button>
            </div>
          </div>
        </div>
...
export class TechniqueEntretiensComponent implements OnInit {
  ...
  loadApplications(): void {
    this.api.get<TechApplication[]>('/technique/shortlists').subscribe(all => {
      const inTech = all.filter(x => x.currentStage === 'TECH_INTERVIEW');
